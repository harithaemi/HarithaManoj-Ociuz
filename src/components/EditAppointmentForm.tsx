...
type EditAppointmentFormProps = {
  ...
  onClose: () => void;
  darkMode: boolean;
  services: Service[]; // ✅ to show service dropdown like add form
};
    ...
    <div className="fixed inset-0 bg-black/50 flex justify-center items-center">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col">
          ...
          />
          {errors.name && (
            <p className="text-red-400 text-sm px-2">{errors.name}</p>
          )}
          ...
          />
          {errors.date && (
            <p className="text-red-400 text-sm px-2">{errors.date}</p>
          )}
